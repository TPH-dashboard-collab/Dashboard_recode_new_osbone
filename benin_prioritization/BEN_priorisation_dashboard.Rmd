...
# Shapefiles com sf (moderno)
Admin2 <- st_read("./Data_prio/shapefiles/communes.shp", quiet = TRUE)

# Simplificar geometria
Admin2_rg <- st_simplify(Admin2, dTolerance = 0.02, preserveTopology = TRUE)
...
Admin2_gSimplify <- Admin2_rg %>%
  left_join(Admin2_df, by = names(Admin2_df)[names(Admin2_df) != "sub" & names(Admin2_df) != "Admin1"])

# Processar ZS
AdminZS <- st_read("./Data_prio/shapefiles/BEN_ZS_dissolved_Shp_BEN.shp", 
                   quiet = TRUE)

# Unir geometrias de Cotonou e criar AdminZS_df
AdminZS_df <- AdminZS %>%
  st_drop_geometry() %>%
  mutate(
    ZS = ifelse(grepl("COTONOU", ZS), "COTONOU", ZS),
    Admin1 = case_when(
      NEW_DEPT == "Ou?m?" ~ "Oueme",
      TRUE ~ NEW_DEPT
    )
...
# Unir geometrias
AdminZS_COT <- AdminZS %>%
  mutate(ZS = ifelse(grepl("COTONOU", ZS), "COTONOU", ZS)) %>%
  group_by(ZS) %>%
  summarise(geometry = st_union(geometry), .groups = "drop")

# Simplificar
AdminZS_rg <- st_simplify(AdminZS_COT, dTolerance = 0.02, preserveTopology = TRUE)
