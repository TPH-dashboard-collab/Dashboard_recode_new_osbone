...
risk_strata = read.csv("/scicore/home/pothin/orlend0000/countryModeling/Tanzania/SNTanalysis/phase0n1/risk_strata.csv")

sim_plan = read_csv2("/scicore/home/pothin/orlend0000/countryModeling/Tanzania/SNTanalysis/phase0n1/Tz_NSP_latest_ST_26_08_2024.csv")
...
TZA_data <- readRDS(
  "/scicore/home/pothin/GROUP/Tanzania_2026/RISK STRATA/RiskStrataInputs.rds"
)
...
nb_dist_pr_NSP_IPTsc = TZA_data |>
  filter(plan %in% planInterested[c(11,10)],
         ...
         year %in% c(2020,2030)) |> #available in postprocessing |>
  group_by(year, plan) |>
  summarise("very low" = sum(ind_sup<=0.019),
            "low" = sum(ind_mean>0.019 & ind_mean<=0.033),
            "moderate" = sum(ind_mean>0.033&ind_mean<=0.11),
            "high"= sum(ind_mean>0.11)) |>
  pivot_longer(cols = 3:6)
